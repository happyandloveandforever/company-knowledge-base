...
| 已导入文件 | **29** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 127 条；迷走三套 56 条已批准；原子库零件箱 20 + 疗法五维脊柱 13；NSF 标准五问 8 条通识卡。仍缺锁数字与真实案例。**产品开发（非专利）：** `docs/舱内壳体传声-产品开发方案.md` |
| **独立专利库 `PAT-*`** | **197** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 145 / 已取代 21 / 已打掉 18 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v4.md` + docx（`PAT-MAP-007`，含全部 87 条先案与 18 条红灯）**。技术总图：`patent-drafts/信噪比架构.md`。申请策略：`PAT-BATCH-002`。新思路入库：`patent-drafts/新思路入库流程.md`（`PAT-RULE-008`）。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
├── docs/舱内壳体传声-产品开发方案.md ← 壳体传声产品开发（效果原理/采购/实验/验收）
├── .cursor/
