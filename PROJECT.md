...
> **最后更新：** 2026-08-25（由 Agent 维护，每次入库后更新此节）

| 指标 | 数值 |
|------|------|
| 知识点总数 | **498**（480 `approved` + 18 `draft`） |
| 通识层 `commons` | **150** 条：公开科学/政策/卫生/证据边界 |
| 公司自有层 `company` | **348** 条：产品、SOP、报价、案例、财务、内训教材 |
| 用途 `usage` | `pitch` 254 · `training` 125 · `ops` 57 · `both` 62 |
| **仅内训 `internalOnly`** | **103** 条（全部 `KP-TRN-*`），编排 PPT 默认排除 |
| 已导入文件 | **25** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 103 条；已补「恢复力=迷走弹性」通识与幻灯片口径。仍缺锁数字与真实案例 |
...
`漂浮培训大纲.docx`（8章35节）已拆为 **`KP-TRN-001~071`**，后又补 **`KP-TRN-072~094`**（好转反应 / 适应症 / 禁忌症 / 产品手册），再补 **`KP-TRN-095~101`**（焦虑 / 跑步 / 交感反弹）与 **`KP-TRN-102~103`**（迷走弹性口径），与其它来源刻意分开：
...
| 自主神经补训：焦虑跑步与交感反弹 | SRC-TRN-ANS-RUN | KP-TRN-095 ~ 101 | 7 | done，已批准；通识层仅内训 | claude-agent |
| 迷走弹性与漂浮方舟：恢复力机理 | SRC-VAGUS-ELASTIC | COM-019~022 + WEB-013 + CRAFT-026 + TRN-102~103 | 8 | done，已批准 | claude-agent |

### 结构体检（2026-08-24）

| 层 | 现状 | 建议 |
|----|------|------|
| 通识层 | 已把文献/政策/卫生从「已批准大锅」里分开；补 WHO 2025、REST 综述、**恢复力=迷走弹性**（COM-019~022、WEB-013） | 客户资料包先抽通识层；095–103 内训卡不外发；幻灯片用 CRAFT-026 |
| 公司层 | SOP/报价/产品/案例仍在此层 | 数字未锁定继续用「合同/铭牌」口径 |
| 培训 vs 汇报 | `usage` 已分开；`KP-TRN-*` 103 条硬隔离；禁忌/手册另有 ops 卡 | 裁定剩余 draft；职业发展与问题集仍空 |
...
- [x] 补训焦虑/跑步/自主神经：TRN-095~101（通识层机制，仅内训）；库 483→490
- [x] 补「恢复力=迷走弹性」：COM-019~022、WEB-013、CRAFT-026、TRN-102~103；库 490→498
...
│   ├── import-anxiety-running-ans.mjs  焦虑/跑步/交感反弹 7 条内训卡（幂等）
│   ├── import-vagus-elasticity.mjs     迷走弹性/恢复力 8 条（幂等）
...
| 2026-08-25 | 补训焦虑/跑步/自主神经 7 条 `KP-TRN-095~101`（通识层机制，仅内训，不贬低跑步、不宣称治焦虑）；库 483→490 |
| 2026-08-25 | 补「恢复力≠让人恢复」：迷走弹性 COM-019~022 + Flux 2022 WEB-013 + 幻灯片口径 CRAFT-026 + 内训 TRN-102~103；库 490→498 |
| 2026-08-25 | 迷走接手窗口透明底 SVG：`public/diagrams/ans-handover-window.svg`（完整）与 `-middle.svg`（插三框与底栏之间） |
