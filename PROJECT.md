...
| **独立专利库 `PAT-*`** | **196** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 144 / 已取代 21 / 已打掉 18 / 待重估 13）。给人看的方案：`patent-drafts/信噪比架构.md`。申请策略：`PAT-BATCH-002`。新思路入库：`patent-drafts/新思路入库流程.md`（`PAT-RULE-008`）。网页：https://company-knowledge-base-nine.vercel.app/patents |
