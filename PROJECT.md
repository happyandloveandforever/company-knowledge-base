...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **379**（375 `approved` + 4 `draft`） |
| 通识层 `commons` | **112** 条：公开科学/政策/卫生/证据边界（含原 WEB/MEV/MEDW 及从核准库拆出的非专有卡 + 18 条 `KP-COM-*`） |
| 公司自有层 `company` | **267** 条：产品、SOP、报价、案例、财务、合作方案 |
| 用途 `usage` | `pitch` 汇报/提案 · `training` 培训 · `ops` 运营SOP · `both` 两用 |
| 已导入文件 | **18** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 已按「通识 / 公司」两层切开。培训不是第三层。仍缺锁数字与真实案例 |

### 两层怎么用（B端资料）

| 层 | 给客户吗 | 典型内容 |
|----|----------|----------|
| **通识层** | 默认可以进资料包 | WHO/政策、REST文献、卫生标准、证据边界、培训红线 |
| **公司自有层** | 按尽调/签约授权 | 产品参数、报价、SOP、具名案例、财务模型 |

培训初稿和 PPT 汇报**不要建成第三层**。同一主题拆两条：`usage=training`（学习目标/误区/练习/考核）和 `usage=pitch`（主张/证据/边界/邀请）。科学培训进通识层，SOP/产品培训进公司层。有培训初稿时上传后说「按培训卡结构拆」。
...
| 培训 vs 汇报 | `usage` 已分开；培训初稿尚未上传 | **请上传培训初稿**，按培训卡结构拆 |
...
- [ ] 上传培训初稿，按 `usage=training` 拆分（科学→通识，SOP/产品→公司）
