...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **490**（472 `approved` + 18 `draft`） |
| 通识层 `commons` | **143** 条：公开科学/政策/卫生/证据边界 |
| 公司自有层 `company` | **347** 条：产品、SOP、报价、案例、财务、内训教材 |
| 用途 `usage` | `pitch` 253 · `training` 123 · `ops` 57 · `both` 57 |
| **仅内训 `internalOnly`** | **101** 条（全部 `KP-TRN-*`），编排 PPT 默认排除 |
| 已导入文件 | **24** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 101 条；禁忌症与产品手册另有运营卡。仍缺锁数字与真实案例 |
...
`漂浮培训大纲.docx`（8章35节）已拆为 **`KP-TRN-001~071`**，后又补 **`KP-TRN-072~094`**（好转反应 / 适应症 / 禁忌症 / 产品手册），再补 **`KP-TRN-095~101`**（焦虑 / 跑步 / 交感反弹，通识层机制），与其它来源刻意分开：
...
| 层 | 现状 | 建议 |
|----|------|------|
| 通识层 | 已把文献/政策/卫生从「已批准大锅」里分开；补 WHO 2025、Atlas 2024、2025/2026 REST 综述；内训补 7 条焦虑/跑步/交感机制 | 客户资料包先抽通识层；在研 NCT 无结果不进 PPT；095–101 虽是通识也不外发 |
| 公司层 | SOP/报价/产品/案例仍在此层 | 数字未锁定继续用「合同/铭牌」口径 |
| 培训 vs 汇报 | `usage` 已分开；`KP-TRN-*` 101 条硬隔离；禁忌/手册另有 ops 卡 | 裁定剩余 draft；职业发展与问题集仍空 |
