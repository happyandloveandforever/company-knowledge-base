...
| **独立专利库 `PAT-*`** | **164** 条（全部 `internal`，不进 `/open`、不进编排）。给人看的当前方案：`patent-drafts/真实技术保护.md`（`PAT-MAP-005`）。网页：https://company-knowledge-base-nine.vercel.app/patents |

### 专利库为什么单独隔离

> **给人看的当前总菜单：`patent-drafts/真实技术保护.md`**（`PAT-MAP-005`，取代 `PAT-MAP-004`）。
>
