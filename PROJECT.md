...
| **独立专利库 `PAT-*`** | **30** 条（全部 `internal`，不进 `/open`、不进编排）。重构版六簇 + 两件母案：总图/规则 + A/B + 状态机 + 不设母案3 + 6 簇结论 + 12 高风险前案 + 红绿清单 + 批次/缺口 |
...
- [ ] 专利库下一步（技术组，不是再检索）：锁 BOM；六类工程测试；核验 CN121795911A 等法律状态；写母案 A/B 交底；B6/B7 先预埋不另立母案3
