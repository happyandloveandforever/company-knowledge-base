...
| **独立专利库 `PAT-*`** | **184** 条（全部 `internal`，不进 `/open`、不进编排）。给人看的当前方案：`patent-drafts/信噪比架构.md`（`PAT-MAP-006`）。申请策略：`PAT-BATCH-002`。网页：https://company-knowledge-base-nine.vercel.app/patents |
