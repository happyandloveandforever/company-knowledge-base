...
| **独立专利库 `PAT-*`** | **68** 条（全部 `internal`，不进 `/open`、不进编排）。**报告 v2.0**：去盐测试闸门 + 母案B收窄 + 取消20件硬指标 + 清水对照组要求；六簇与两母案结构不变；交底书撰写包 7 条；中外前案 25 条（含美国专利与 IEC 标准） |
...
### 五个候选角度（`PAT-IDEA-001`~`005`，全部待检索/待验证）

| 角度 | 靠哪条 | 评估 |
|---|---|---|
| ① 反用感官剥夺做**设备**故障预测 | E3＋E5 | 🟢 最看好，不涉人体不碰第25条 |
| ② 中性浮力下无接触面体征测量 | E2 | 🔴 手段拥挤，US6669649 已占浮力测呼吸 |
| ③ 声阻抗接近→**声能剂量安全** | E2＋E4 | 🟢 写安全不写疗效 |
| ④ 迷走走装置路线 | E1＋E2 | 🟡 法律有空间证据不足，二期 |
| ⑤ **高盐是唯一可调的密度旋钮** | E1＋E2 | 🟢 成本项变功能项 |
| ⑥ 把成熟迷走技术接入本舱做**组合发明** | 待定 | 🟡 路通，协同点未锁定 |
...
| **高盐结晶致传感器沉默失效** | ✅ 仍成立 | 未检索到 |

独权须以最后一条重构。**下一步是实验**：高盐环境下传感器多久被盐晶糊住、糊住后读数是归零还是卡住。测不出这个现象，本案要重选。
...
- [ ] **优先实验**：高盐 vs 清水对照，传感器多久被盐晶糊住、糊住后读数特征。决定 `PAT-DRAFT-A4` 生死
