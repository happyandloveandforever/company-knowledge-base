...
| **独立专利库 `PAT-*`** | **232** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 173 / 已取代 21 / 已打掉 25 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v5.md`。** **撰写：`PAT-WRITE-007` 现为「现机不递；改机后写气相消杀与通气孔分态；余光暂停」。** 交底书 `patent-drafts/交底书-空舱气相消杀-改机.md`。现机结论仍见 `patent-drafts/舱体消杀-现机可写点.md`。应用对照：`PAT-MAP-009`。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
- **正在写的件**：改机后的空舱气相消杀（通气孔分态）。交底书下载：首页「下载交底书」，或 `/api/download?file=jiaodishu.docx`（文件名 `漂浮方舟_交底书.docx`）。底稿 `patent-drafts/交底书-空舱气相消杀-改机.md`
...
- **2026-09-06 改机**：用户愿意改机器。只写一件：通气孔分态 + 臭氧改接气相并切断液相 + 光触媒 + 占用硬断电 + 气相臭氧探头。1–6 缺一项不递。不写灭活率。余光仍暂停。
- **写的是交底书，不是权利要求书**：见 `PAT-WRITE-002`；独权由代理师定稿
- **硬规则**：工程师没说过的结构、参数、阈值、数据一律不写；厂家说明书上的合格限可以抄并注明出处
