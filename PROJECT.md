...
| **独立专利库 `PAT-*`** | **54** 条（全部 `internal`，不进 `/open`、不进编排）。**报告 v2.0**：去盐测试闸门 + 母案B收窄 + 取消20件硬指标 + 清水对照组要求；六簇与两母案结构不变；交底书撰写包 7 条；中外前案 25 条（含美国专利与 IEC 标准） |
...
### 去盐测试：立项前必做（`PAT-RULE-003`）

把方案描述里的「高盐 / 漂浮液 / 漂浮舱」换成「液体 / 容器 / 密闭舱」，再问一句方案是不是照样成立。

- 🟢 完全不成立 → 高盐是必要条件 → 可立案，优先
- 🟡 仍成立但效果显著变差 → 需对照数据后立案
- 🔴 照样成立 → 通用工程手段 → 不立案，最多作从权

理由：通用安全工程（IEC 60079／61508 体系）全球积累远超我们，**高盐是唯一护城河**。已验证：第一件底稿四个发明点，三个红灯被打掉，剩下的正是唯一绿灯。

据此 **母案A 升、母案B 降**。母案B 从「激励—测量抗干扰与设备安全状态控制」收窄为「高盐环境下的测量可信度维持与设备安全控制」。
...
│   ├── apply-patent-report-v2.mjs      报告v2.0结论：去盐测试、母案B收窄
