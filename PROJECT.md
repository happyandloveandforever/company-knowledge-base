...
| **独立专利库 `PAT-*`** | **232** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 173 / 已取代 21 / 已打掉 25 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v5.md` + docx（`PAT-MAP-008`）**。**现在就能写：`PAT-WRITE-007` 经现机过闸后，空舱三态按现状不递，现行第一件为占用态余光+通气孔。** 应用对照：`PAT-MAP-009` + `PAT-XREF-002`。技术总图：`PAT-MAP-006`。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
- **现在就能写的菜单**：`PAT-WRITE-007` + `patent-drafts/无需实验即可撰写的专利方案.md`（首页可下载 Word）
- **2026-09-06 现机过闸**：有通气孔、臭氧接液体、无涂层、冲水不结盐、舱内紫外、无臭氧探头 → **空舱三态按现状不递**
- **现行第一件**：占用态余光 + 通气孔。交底书 `patent-drafts/交底书-时变余光限制装置.md`
- **原件1交底书**保留作否决留痕，不要再交给代理师
...
| 日期 | 变更 |
|------|------|
| 2026-09-06 | **现机过闸**：通气孔/臭氧接液体/无涂层/不结盐/舱内紫外/无臭氧探头。空舱三态按现状不递；`PAT-WRITE-007` 改为占用态余光+通气孔为现行第一件。不新立 IDEA |
