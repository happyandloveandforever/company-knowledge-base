...
| **独立专利库 `PAT-*`** | **233** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 174 / 已取代 21 / 已打掉 25 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v5.md`。** **撰写：申请正文底稿 `patent-drafts/申请文件底稿-空舱气相消杀.md`（`PAT-DRAFT-028`）。** 首页「下载申请正文」。交底书仍可下。余光暂停。应用对照：`PAT-MAP-009`。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
### 第一件专利怎么写（`PAT-WRITE-007`；旧包 `001~006` 仍可参考）

用户是非工程背景，负责采访与结构化，不负责发明。**明确不要先做实验时，不要把组二 `046` 当第一件**（那两张表就是实验）：

- **正在写的件**：新机空舱气相消杀。申请正文 `patent-drafts/申请文件底稿-空舱气相消杀.md`，状态卡 `PAT-DRAFT-028`。首页「下载申请正文」`/api/download?file=shuomingshu.docx`
- **2026-09-06 现机过闸**：有通气孔、臭氧接液体、无涂层、冲水不结盐、舱内紫外、无臭氧探头 → **空舱三态按现机不递**
- **2026-09-06 再改判**：余光件暂停。舱体消杀现机无独立成件点（冲壁/紫外/臭氧进液均为公开标配）。结论 `patent-drafts/舱体消杀-现机可写点.md`
- **2026-09-06 改机**：用户确认新机已改齐。申请正文按新机实施例写。独权：通气孔分态 + 臭氧改接气相并切断液相 + 三态互斥。不写灭活率。余光仍暂停。
- **申请正文按中国发明专利结构写**：说明书、权利要求书、摘要。权要由代理师定稿（`PAT-WRITE-002`）。本件是专利申请文件，不是著作权登记。
- **硬规则**：不编 ppm 和杀菌率；厂家说明书上的合格限可以抄并注明出处
- 旧模板 `patent-drafts/交底书-母案A.md` 与 `PAT-WRITE-001`（已取代）不再作为第一件指引
...
- [x] 两库合并：专利对照 PAT-MAP-009 + PAT-XREF-002；知识库 RD-001/012 互挂；不改五个申请组
...
│   ├── apply-rd-two-library-merge.mjs  两库合并：KP-RD 注脚 + PAT-MAP-009 / PAT-XREF-002
...
| 日期 | 变更 |
|------|------|
| 2026-09-06 | **申请正文底稿**：用户确认新机已改齐。按中国发明专利结构写出说明书+权利要求书+摘要。`PAT-DRAFT-028`（232→233，现行 173→174）。首页「下载申请正文」。权要代理师定稿。余光仍暂停 |
| 2026-09-06 | **交底书下载入口**：首页主按钮改为「下载交底书」`/api/download?file=jiaodishu.docx`，与「怎么写」方案分开，避免两个链接都叫改机 |
| 2026-09-06 | **舱体消杀改机撰写**：用户愿意改机器。现机仍不递 028。改机后只写一件：通气孔分态 + 空舱气相处理。交底书 `patent-drafts/交底书-空舱气相消杀-改机.md`。余光仍暂停。专利卡仍 232 |
| 2026-09-06 | **舱体消杀过闸**：余光件暂停。现机冲壁/紫外/臭氧进液/排空均为公开标配，`028` 零件不齐，舱体消杀无独立成件点。唯一待问冲洗水去向 |
| 2026-09-06 | **现机过闸**：通气孔/臭氧接液体/无涂层/不结盐/舱内紫外/无臭氧探头。空舱三态按现状不递；随后余光件亦暂停，见上行 |
| 2026-09-06 | **无需实验撰写菜单**：`PAT-WRITE-007`（231→232，现行 172→173）。随后被同日现机过闸改判，见上行 |
| 2026-09-05 | **两库合并**：知识库 `KP-RD-001~012`（592 条）+ 专利库对照 `PAT-MAP-009`、`PAT-XREF-002`（229→231，现行 170→172）。不新写权要，不改五个申请组，不取代 MAP-006/008。Word 仍可从首页下载 |
