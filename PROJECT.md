...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **660**（642 `approved` + 18 `draft`） |
| 通识层 `commons` | **227** 条：公开科学/政策/卫生/证据边界 + 迷走/VNS 通识 + NSF 标准五问 + 罗森堡读本 1–160 页 |
| 公司自有层 `company` | **433** 条：产品、SOP、报价、案例、财务、内训教材、综合干预专业版、内部原子库、疗法叙事脊柱、罗森堡对接口径 |
| 用途 `usage` | `pitch` 326 · `training` 185 · `ops` 61 · `both` 88 |
| **仅内训 `internalOnly`** | **127** 条（`KP-TRN-*` 94 + `KP-ATOM-*` 20 + `KP-RX-*` 13），编排 PPT 默认排除 |
| 已导入文件 | **33** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 127 条；迷走三套 56 条已批准；罗森堡读本 80 条（框架+解剖+五态/神经觉+第4章检测启发式+第5章 COPD 开头）；原子库零件箱 20 + 疗法五维脊柱 13；NSF 标准五问 8 条通识卡。仍缺锁数字与真实案例 |
...
- [ ] 罗森堡书后续页：优先 161 页起（第5章 COPD/裂孔疝收束与更多病谱）；自助动作另包，按培训卡拆步骤
...
| `/open/catalog` | 来源清单：总库已合并 660，可外发 533，仅内训 127 不收录 |
