...
| **独立专利库 `PAT-*`** | **232** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 173 / 已取代 21 / 已打掉 25 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v5.md`。** **撰写：`PAT-WRITE-007` 现为「空舱三态不递、余光暂停、舱体消杀现机无独立件」。** 结论稿 `patent-drafts/舱体消杀-现机可写点.md`。应用对照：`PAT-MAP-009`。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
- **现在没有正在写的件**：菜单 `PAT-WRITE-007` + `patent-drafts/无需实验即可撰写的专利方案.md`
- **2026-09-06 现机过闸**：有通气孔、臭氧接液体、无涂层、冲水不结盐、舱内紫外、无臭氧探头 → **空舱三态按现状不递**
