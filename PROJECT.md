...
> **最后更新：** 2026-08-29（由 Agent 维护，每次入库后更新此节）

| 指标 | 数值 |
|------|------|
| 知识点总数 | **483**（465 `approved` + 18 `draft`） |
| 通识层 `commons` | **136** 条：公开科学/政策/卫生/证据边界 |
| 公司自有层 `company` | **347** 条：产品、SOP、报价、案例、财务、内训教材 |
| 用途 `usage` | `pitch` 253 · `training` 116 · `ops` 57 · `both` 57 |
| **仅内训 `internalOnly`** | **94** 条（全部 `KP-TRN-*`），编排 PPT 默认排除 |
| 已导入文件 | **23** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 94 条；禁忌症与产品手册另有运营卡。仍缺锁数字与真实案例。已出《漂浮知识手册》内容版 HTML |
...
- [x] 对照旧稿《漂浮疗法知识手册》重写内容版（HTML，可自行转 PDF）
...
| 2026-08-29 | 新版《漂浮知识手册》内容稿：旧 40 页手册按知识库红线重写为 HTML（`public/exports/float-handbook-v2.html`），未把旧稿过声称拆进知识点 |
