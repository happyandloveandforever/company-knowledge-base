...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **484**（466 `approved` + 18 `draft`） |
| 通识层 `commons` | **136** 条：公开科学/政策/卫生/证据边界 |
| 公司自有层 `company` | **348** 条：产品、SOP、报价、案例、财务、内训教材 |
| 用途 `usage` | `pitch` 254 · `training` 116 · `ops` 57 · `both` 57 |
| **仅内训 `internalOnly`** | **94** 条（全部 `KP-TRN-*`），编排 PPT 默认排除 |
| 已导入文件 | **24** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 94 条；已补北化「四条价值轴」白话口径（KP-CRAFT-027）。仍缺锁数字与真实案例 |
...
| 四条价值轴白话改写 | SRC-FOUR-AXES | KP-CRAFT-027 | 1 | done，已批准 | 口径改写 |
...
| 2026-08-25 | 北化 PPT「四条价值轴」白话改写：`KP-CRAFT-027`；大纲第12页去掉「载体赋能 / 健康管理表达」；库 483→484 |
