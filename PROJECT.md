...
| **独立专利库 `PAT-*`** | **232** 条（全部 `internal`，不进 `/open`、不进编排）。**入口先读 `PAT-INDEX-001`**（现行 173 / 已取代 21 / 已打掉 25 / 待重估 13）。**对外交付：`patent-drafts/专利整合方案-v5.md` + docx（`PAT-MAP-008`，含全部 97 条先案与 25 条红灯）**。**现在就能写、不必实验：`PAT-WRITE-007` + `patent-drafts/无需实验即可撰写的专利方案.md`（首选空舱三态一件）**。应用对照：`PAT-MAP-009` + `PAT-XREF-002`（对应知识库 `KP-RD-*`，不新写权要）。技术总图：`patent-drafts/信噪比架构.md`（`PAT-MAP-006`）。申请策略：`PAT-BATCH-002`。本轮过闸：`PAT-EXT-004`、`PAT-EXT-005`。新思路入库：`patent-drafts/新思路入库流程.md`（`PAT-RULE-008`）。网页：https://company-knowledge-base-nine.vercel.app/patents |
...
- **首选件1**：空舱三态消杀装置（占用 / 空舱气相处理 / 回填互斥）。交底书 `patent-drafts/交底书-空舱三态消杀装置.md`
- **可选件2**：时变余光限制。交底书 `patent-drafts/交底书-时变余光限制装置.md`
...
| 日期 | 变更 |
|------|------|
| 2026-09-06 | **无需实验撰写菜单**：`PAT-WRITE-007`（231→232，现行 172→173）。首选空舱三态一件，可选时变余光；046 两张表未做不作第一件独权。交底书三份 + 首页 Word 下载。不新立 IDEA |
