...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **450**（436 `approved` + 14 `draft`） |
| 通识层 `commons` | **136** 条：公开科学/政策/卫生/证据边界 |
| 公司自有层 `company` | **314** 条：产品、SOP、报价、案例、财务、内训教材 |
| 用途 `usage` | `pitch` 253 · `training` 93 · `ops` 47 · `both` 57 |
| **仅内训 `internalOnly`** | **71** 条（全部 `KP-TRN-*`），编排 PPT 默认排除 |
| 已导入文件 | **19** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；首页「知识库入口」第三张卡为漂浮师内训；培训教材独立命名空间并硬隔离。仍缺锁数字与真实案例 |
...
`漂浮培训大纲.docx`（8章35节）已拆为 **`KP-TRN-001~071`**，与其它来源刻意分开：
...
| 培训 vs 汇报 | `usage` 已分开；培训大纲 71 条已入库并硬隔离 | 先裁定 10 条 draft，再补设备实操与问题集 |
...
- [ ] **裁定 10 条培训 draft**：属相择时/治愈反应/气冲病灶/气的物理表述/中医解释/透皮吸收率/适应症清单/好转反应/处方体例/19例统计
- [ ] 补培训教材四处空白：设备使用指南、理疗部件、漂浮师职业发展、问题集（见 KP-TRN-068）
