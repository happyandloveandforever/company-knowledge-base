...
| 指标 | 数值 |
|------|------|
| 知识点总数 | **615**（597 `approved` + 18 `draft`） |
| 通识层 `commons` | **182** 条：公开科学/政策/卫生/证据边界 + 迷走/VNS 通识 + NSF 标准五问 + 罗森堡读本 35 条（1–200 合并 15 + 201–240 续拆 20） |
| 公司自有层 `company` | **433** 条：产品、SOP、报价、案例、财务、内训教材、综合干预专业版、内部原子库、疗法叙事脊柱、罗森堡对接口径 |
| 用途 `usage` | `pitch` 308 · `training` 172 · `ops` 62 · `both` 73 |
| **仅内训 `internalOnly`** | **127** 条（`KP-TRN-*` 94 + `KP-ATOM-*` 20 + `KP-RX-*` 13），编排 PPT 默认排除 |
| 已导入文件 | **36** |
| 待 Claude 拆分队列 | 0 |
| 结构判断 | 两层已切开；内训 127 条；迷走三套 56 条已批准；罗森堡读本 **35 条**（001~015 为 1–200 主题合并；016~035 为 201–240 续拆，待 241 页）；原子库零件箱 20 + 疗法五维脊柱 13；NSF 标准五问 8 条通识卡。仍缺锁数字与真实案例 |
...
| 迷走神經的自我檢測與治癒_201-240.pdf | SRC-PVB-ROSENBERG-201-240 | KP-PVB-016 ~ 035 | 20 | done，已批准；待续 241 | claude-agent |
...
| 罗森堡读本 201–240 续拆（2026-09-06） | 20 条：偏头痛激痛点收束 + 第6章身心/焦虑/回盲瓣/家暴/PTSD | 不药而愈、内脏按摩、家暴/PTSD 不得当适应症或门店 SOP；待 241 页 |
...
- [x] 同书 201–240 页续拆为 KP-PVB-016~035（偏头痛收束 + 第6章至 PTSD 开篇；待 241 页）
...
- [x] 罗森堡书 1–200 页已主题合并为 15 条；201–240 已续拆为 016~035；勿再跑 1–200 逐页 import
...
| `/open/catalog` | 来源清单：总库已合并 615，可外发 488，仅内训 127 不收录 |
