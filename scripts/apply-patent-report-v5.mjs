...
本轮补上第二句：**平移的是自由漂浮才出现的物理问题，不是床、灯、气、冥想舱的现成模块。** 床靠固定接触面；漂浮舱没有固定接触面。浴缸可以造浪；漂浮舱还要管口鼻附近的自由液面、空气声、壳体声和 45–90 分钟的累积漂移。SPA 可以照光、溶氢、对置喷水；那些在固定座位里成立的方案，拿到近中性浮力的自由漂浮体上，要么已经有人写过，要么发明点根本不在种类和波长上。继续按五个申请组写，不恢复母案称谓，不另立「光疗组」或「体感音乐组」。组二的两张表仍然是整个库最该立刻做的事。本轮新开的结构台架穿插在第二、第三优先：零合力投送（\`PAT-IDEA-069\`）、拉格朗日漂移（\`PAT-IDEA-060\`）、光窗物态（\`PAT-IDEA-067\`）、气体交叉误判（\`PAT-IDEA-070\`），清单见 \`PAT-GAP-009\`。
...
改按贡献点分组，组内用从属权利要求做层次。本轮两份评估建议另立「液体律动场 / 干湿界面 / 光路可信 / 含气液投送」——那是研发筛选顺序，不是现在改架构。新点全部挂进既有五组。不要把 \`PAT-IDEA-064\` 写成姿态调盐度或人体信号策略库。
